...
export default function RecipeManager() {
  ...
  function reset() { setEditing(null); setName(""); setOutputItemId(""); setEffectiveFrom(new Date().toISOString().slice(0, 10)); setLines([{ itemId: "", quantityGrams: "" }]); }
  function edit(recipe: any) { setEditing(recipe); setName(recipe.name); setOutputItemId(recipe.outputItemId ?? ""); setEffectiveFrom(String(recipe.effectiveFrom).slice(0, 10)); setLines(recipe.lines.length ? recipe.lines.map((line: any) => ({ itemId: line.itemId, quantityGrams: String(line.quantityGrams) })) : [{ itemId: "", quantityGrams: "" }]); }
  function submit(event: FormEvent) { event.preventDefault(); if (!name.trim()) return; save.mutate({ id: editing?.id, name: name.trim(), outputItemId: outputItemId ? Number(outputItemId) : null, outputQuantityGrams: 1, note: null, active: true, effectiveFrom, lines: lines.filter(line => line.itemId && Number(line.quantityGrams) > 0).map(line => ({ itemId: Number(line.itemId), quantityGrams: Number(line.quantityGrams) })) }); }
